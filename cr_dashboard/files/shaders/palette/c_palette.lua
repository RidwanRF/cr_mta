...
paletteTable = { customPalette = dxCreateTexture("files/shaders/palette/palette/enbpalette1.png"), palEffectEnabled = false, paletteEnabled = true, 
...
function enablePalette(number)
	...
	-- Shaders
	chromaticShader = dxCreateShader( "files/shaders/palette/fx/chromatic.fx" )
	paletteShader = dxCreateShader( "files/shaders/palette/fx/palette.fx" )
	...
	if not isElement( paletteTable.customPalette ) then 
		chosenPalette = dxCreateTexture("files/shaders/palette/palette/enbpalette" .. number .. ".png")
	else
