...
function createHifi(creator, x,y,z)
    if isElement(creator) and creator:getData("acc >> id") then
        ...
        dbExec(connection, "INSERT `hifi` SET `x`=?, `y`=?, `z`=?, `rotZ`=?, `creator`=?", x,y,z, 0, creator:getData("acc >> id"))
        ...
        connection, "SELECT * FROM `hifi` WHERE `x`=? and `y`=? and `z`=? and `rotZ`=? and `creator`=?", x,y,z, 0, creator:getData("acc >> id"))
    end
end
