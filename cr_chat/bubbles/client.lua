...
function toggleBubbles(cmd)
    ...
    if not value["disabled"] then
        ...
        outputChatBox(syntax.. "Sikeresen bekapcsoltad a buborékokat!", 255,255,255,true)
        addEventHandler("onClientRender", root, drawnBubbles, true, "low-5")
    else
        local syntax = exports['cr_core']:getServerSyntax(false, "success")
        outputChatBox(syntax.. "Sikeresen kikapcsoltad a szöveg buborékokat!", 255,255,255,true)
...
local fontsize = 1
local font = exports['cr_fonts']:getFont("Roboto", 12)
addEventHandler("onClientResourceStart", root,
    function(startedRes)
        if getResourceName(startedRes) == "cr_fonts" then
            font = exports['cr_fonts']:getFont("Roboto", 12)
        end
    end
)
...
function drawnBubbles()
    if getPedOccupiedVehicle(localPlayer) then
        --if getElementData(getPedOccupiedVehicle(localPlayer), "veh >> windowState") then return end
    end
