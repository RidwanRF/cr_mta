...
function playerTime(e, sectorDetails)
    ...
    local color, white = exports['cr_core']:getServerColor(nil, true), "#ffffff"
    
    if playerTimes[1] and playerTimes[1][1] then
        if time <= playerTimes[1][1] then
            outputChatBox(exports['cr_core']:getServerSyntax("Versenypálya", "success") .. color .. playerName .. white .." megdöntötötte a csúcsidőt. Új csúcsidő: ".. color ..formatIntoTime(time), root, 255,255,255,true)
        end
    else
        outputChatBox(exports['cr_core']:getServerSyntax("Versenypálya", "success") .. color .. playerName .. white .." megdöntötötte a csúcsidőt. Új csúcsidő: ".. color ..formatIntoTime(time), root, 255,255,255,true)
    end
...
function ghostMode(e, state)
    triggerClientEvent(root, "ghostMode", e, e, state)
end
