...
setTimer(
    function()
        ...
        for k,v in pairs(attached_ped) do
            local _v = v
            if getElementData(v, "clone") and isElement(getElementData(v, "clone")) then v = getElementData(v, "clone") end
            if not isElement(k) then
			    clearAttachmentData(k)
		    elseif isElementStreamedIn(v) and isElementOnScreen(v) or v == localPlayer then
                if k.alpha ~= 255 or k.alpha ~= v.alpha then
                    k.alpha = v.alpha
                end
                renderCache[k] = _v
                
                if k.dimension ~= v.dimension then
                    k.dimension = v.dimension
                end
                
                if k.interior ~= v.interior then
                    k.interior = v.interior
                end
            else
                --setElementPosition(k,getElementPosition(v))
...
function putAttachedElementsOnBones()
	for element,ped in pairs(renderCache) do
        if getElementData(ped, "clone") and isElement(getElementData(ped, "clone")) then ped = getElementData(ped, "clone") end
