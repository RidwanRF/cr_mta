...
function runString(commandstring, outputTo, source)
    ...
    exports['cr_logs']:addLog("Runcode", "crun", "SRUN -> "..sourceName.." lefutatta: "..tostring(commandstring))
...
addCommandHandler("crun", 
    function(player, command, ...)
        ...
        local commandstring = table.concat({...}, " ")
        if player then
            --exports.al_logs:logMessage("Runcode: "..getPlayerName(player).." lefuttatva "..tostring(commandstring).." (client-oldal)", 5)
            exports['cr_logs']:addLog("Runcode", "crun", "CRUN -> "..getPlayerName(player).." lefuttata: "..tostring(commandstring))
            outputDebugString("CRUN -> "..getPlayerName(player).." lefuttata: "..tostring(commandstring), 0, 87, 255, 255)
