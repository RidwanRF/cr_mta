...
components = {
    ...
    ["bump_front_dummy3"] = {false, "Sárhányok megjavítása", {0,0,0}},
        ...
        if isVisible then
                ...
                if getDistanceBetweenPoints3D(pos, localPlayer.position) <= gMaxDist and not disabled then
                    ...
                    if sx and sy then
                        ...
                        if isInSlot(x,y,w,h) then
                            ...
                            dxDrawRectangle(x,y,w,h, tocolor(r,g,b,220))
                            dxDrawText(hunText, x,y,x+w,y+h, tocolor(0,0,0,255), 1, font, "center", "center")
                        else
                            dxDrawRectangle(x,y,w,h, tocolor(20,20,20,220))
                            dxDrawText(hunText, x,y,x+w,y+h, tocolor(255,255,255,255), 1, font, "center", "center")
                        end
                    end
                end
            end
            ...
            if x then
                ...
                if getDistanceBetweenPoints3D(pos, localPlayer.position) <= gMaxDist and not disabled then
                    ...
                    size[1] = size[1] * multipler
                    size[2] = size[2] * multipler
                    local x,y,w,h = sx - size[1] - 20, forY, size[1], size[2]
                    forY = forY + h + 10
                    --hover = {}
                    if isInSlot(x,y,w,h) then
                        hover = {true, v, _k}
                        ...
                        if not doingState then
                            ...
                            if not element:getData(componentName.."->doing") then
                                ...
                                if not isDirectComp[componentName] and isComponentMissing(element, componentName) then
                                    ...
                                    outputChatBox(syntax .. "Menj a sárga cphez, hogy megkaphasd ezt a komponents!",255,255,255,true)
                                    return
                                end
                                ...
                                xS = 0
                                animate(0, 1, "Linear", math.random(5000, 10000), 
                                    function(v)
                                        --outputChatBox(xS)
                                        xS = v
                                    end,
                                ...
                                outputChatBox(syntax .. "Ezt a komponenst nem szedheted le hisz egy kollégád már ezt megtette!", 255,255,255,true)
                            end
                        elseif doingState and doingState == componentName then
                            if element == gVeh then
                                ...
                                if not isComponentDamaged(v, k) then
                                    ...
                                    local syntax = exports['cr_core']:getServerSyntax("Mechanic", "error")
                                    outputChatBox(syntax .. "A komponens amit visszaszeretnél szerelni törött!",255,255,255,true)
                                end
                            else
                                local syntax = exports['cr_core']:getServerSyntax("Mechanic", "error")
                                outputChatBox(syntax .. "Csak arra a kocsira szerelheted fel a komponenst melyről leszedted!", 255,255,255,true)
                            end
                        end
                    end
                end
...
function fixVehComponent(element, k)
    if k == "bonnet_dummy5" then
        ...
        local syntax = exports['cr_core']:getServerSyntax("Mechanic", "warning")
        ...
        outputChatBox("Utolsó feltöltés: "..green..lastOilRecoil,255,255,255,true)
        outputChatBox("Jelenlegi KM óra állás: "..green..odometer,255,255,255,true)
        outputChatBox("Lerobbanás: "..green.. (odometer-lastOilRecoil) .. white .. " KM múlva",255,255,255,true)
        return
    end
...
function doingRender()
    ...
    dxDrawRectangle(x,y,w,h, tocolor(20,20,20,220))
    local r,g,b = exports['cr_core']:getServerColor(nil, false)
    dxDrawRectangle(x,y,w * xS,h, tocolor(r,g,b,220))
    dxDrawText(gText, x,y,x+w,y+h, tocolor(0,0,0,255), 1, font, "center", "center")
end

sx, sy = guiGetScreenSize()
local cursorState = isCursorShowing()
local cursorX, cursorY = sx/2, sx/2
if cursorState then
